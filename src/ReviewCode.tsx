...
export default function ReviewCode() {
  const {
    ...
    result, loading, error, reviewCode,
  } = useReviewCode()

  return (
    <div className="flex flex-col gap-6 p-6 max-w-2xl mx-auto">
      <h1 className="text-xl font-semibold">Review Code</h1>
      ...
      <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-muted-foreground">
        <div className="flex items-center gap-2">
          <label htmlFor="rc-language">Language</label>
          <select
            id="rc-language"
            value={language}
        ...
        <div className="flex items-center gap-2">
          <label htmlFor="rc-focus">Focus</label>
          <select
            id="rc-focus"
            value={focus}
            onChange={e => setFocus(e.target.value as 'general' | 'security' | 'performance' | 'style')}
            disabled={loading}
            className="rounded-md border border-input bg-background px-2 py-1 text-sm"
          >
        ...
        <div className="flex items-center gap-2">
          <label htmlFor="rc-max-tokens">Max tokens</label>
          <Input
            id="rc-max-tokens"
            type="number"
            min={64}
        ...
        <Button onClick={() => void reviewCode()} disabled={loading || !code.trim()}>
